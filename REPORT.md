...
- **Основные метрики:**  
  I. `http_request_duration_ms` — показывает распределение времени ответа, отслеживает p95/p99 и выявляет узкие места  
  II. `container_memory_usage_bytes` — контроль использования памяти контейнерами, предотвращение OOM kills  
  III. `redis_connected_clients` — показывает нагрузку на Redis и выявляет утечки соединений  
- **Скриншот дашборда:**
...
- **Выводы и план улучшений:**  
  I. Внедрить Redis кеширование для снижения p95 < 100 ms  
  II. Оптимизировать пул соединений Redis (maxClients=50, min=10)  
  III. Горизонтальное масштабирование: увеличить количество реплик до 5 с автоскейлингом
